...
class checkLoggingController extends ajaxcontroller {
...
// }
    ...
    public function doAction() {
        ...
        if (substr(php_uname(), 0, 7) == "Windows"){
            log::doLog("windows");
            $ret = pclose(popen("start C:\wamp\bin\php\php5.4.3\php ".INIT::$MODEL_ROOT."/checkLogging.php ".$id_file." ".$id_job." 1", "r"));
        }
        else{
            $ret = pclose(popen("nohup php ".INIT::$MODEL_ROOT."/checkLogging.php ".$id_file." ".$id_job." 1 &", "r"));
        }
        
        log::doLog("CASMACAT: return checkingLogging: ".$ret);
...
//        $this->content = file_get_contents(INIT::$LOG_DOWNLOAD . "/" . $this->filename);
        ...
        if($ret == "END"){
            ...
            $this->result['errors'] = "It is not possible to check the logging";
            $this->result['code'] = -1;
        }
